/*
 * card_auth.c — TrollStore-R 本地卡密校验 Dylib
 ...
 *   H1 强制只生成 -G 通用卡密, 禁止 -B 绑定类型
 *
 * 固有局限（纯本地无服务器，逆向可Hook绕过，清空钥匙串重置本地状态）:
 *   1. H1-G 卡密支持多设备共用; 其余档位可选通用或绑定本机
 ...
 *       -dynamiclib card_auth.c -o card_auth.dylib
 *
...
- (bool)parseAndValidateCard:(NSString *)card_str {
    ...
    /* 2. 强制规则: H1-B 直接判定非法 */
    if ([prefix isEqualToString:@"H1"] && is_bind) {
        [self showErrorAlert:@"H1不支持绑定"
                       message:@"一小时档位(H1)只允许通用(G)卡密, 不存在绑定(B)类型"];
        return false;
    }
